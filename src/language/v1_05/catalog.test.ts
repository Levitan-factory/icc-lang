...
describe("dslCatalog", () => {
  ...
  it("keeps ensemble routing group-only in the preview catalog", () => {
    const modelProfile = dslCatalogEntries.find((entry) => entry.id === "model-profile");
  ...
  it("keeps catalog examples compatible with the current parser", () => {
    const parsed = parseCellDsl(
      ...
      "Use %from c1 and %file.c2:report.md.",
      {
  ...
  it("keeps legacy double-brace migration out of current syntax", () => {
    expect(dslCatalogEntries.find((entry) => entry.id === "legacy-reference-migration")?.status).toBe("legacy");
    expect(dslCatalogEntries.find((entry) => entry.id === "use-artifact")).toBeUndefined();
  });
});
