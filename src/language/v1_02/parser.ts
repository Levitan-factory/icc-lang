...
export function parseCellDsl(
  ...
  for (const [index, rawLine] of headerLines.entries()) {
    ...
    if (line.startsWith("@image")) {
      const directive = parseImageDirective(line, diagnostics, lineNumber);
      if (directive) images.push(directive);
      continue;
    }
...
function parseFileDirective(
  ...
  return parseArtifactDirective("file", requested, line, diagnostics, lineNumber) as FileOutputDirective | undefined;
}
...
function parseImageDirective(
  ...
  diagnostics: Diagnostic[],
  lineNumber: number,
): ImageOutputDirective | undefined {
  ...
  const requested = match[1]?.trim() ?? "";
  return parseArtifactDirective("image", requested, line, diagnostics, lineNumber) as ImageOutputDirective | undefined;
}
...
function parseReferences(
  ...
  while ((match = legacyRegex.exec(text)) !== null) {
    ...
    diagnostics.push({
      level: "warning",
      message: `Legacy reference ${raw} detected. Use ${migrated ?? "ICC v1.02 %from syntax"} instead.`,
    });
